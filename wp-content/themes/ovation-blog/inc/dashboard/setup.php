...
<div class="theme-offer">
  ...
  if(isset($_GET['import-demo'])){
    ...
    for($i=1;$i<=4;$i++){
      ...
      set_theme_mod('ovation_blog_post_setting' . $i, $slider_post_id);

    }

    //-------------- Post Categories----------------------- 

    $post_category = wp_create_category('Post Catgories'); 
    ...
    $top_category = wp_create_category('Top Catgories'); 
    ...
    for($i=1;$i<=6;$i++){
      ...
      $my_post = array(
       ...
       'post_status'   => 'publish',
       'post_type'     => 'post',
       'post_category' => array($top_category) 
      );
  ...
  <p class="note"><?php esc_html_e("If your website is already live and containing data, please make a backup.This importer will override the Ovation Blog's new customizable values.",'ovation-blog'); ?></p>
  <form id="mep-demo-importer-form" action="<?php echo esc_url(home_url()); ?>/wp-admin/themes.php?page=ovation-blog-guide-page" method="POST">
    <input type="submit" name="submit" value="<?php echo esc_attr( __('Begin With Demo Import', 'ovation-blog') ); ?>" class="button button-primary button-large">
    <a href="<?php echo esc_url(home_url('/')); ?>" target="_blank" class="button button-primary button-large"><?php esc_html_e('View Site','ovation-blog'); ?></a>
  </form>
  <div class="mep-spinner-div"><p class="spinner"></p></div>
  <div class="success">
    <?php if (isset($_GET['import-demo'])) {
       echo esc_html(__('Demo Import Successful', 'ovation-blog'));
    } ?>
  </div>
  ...
  <script type="text/javascript">
    ...
    jQuery(document).ready(function($) {
      ...
      if(searchParams) {
        history.replaceState({}, '', 'themes.php?page=ovation-blog-guide-page')
      }
